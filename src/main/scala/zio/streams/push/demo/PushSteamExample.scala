...
import zio.{Chunk, IO, Schedule, Scope, ZIO, ZIOAppArgs, ZIOAppDefault, durationInt}

import java.io.IOException
import ChunkedPushStream._
...
//    }.orElse(ZStream.range(11, 15)).runCollect.flatMap(r => ZIO.succeed(println(s"finished with $r")))
    ...
    val t2: ChunkedPushStream[Any, Throwable, Int] = t1.mapZIOChunks(i => ZIO.attempt(i + 10))
    ...
    val p2: ChunkedPushStream[Any, Throwable, Int] =  p1.scanZIOChunks(0)((state, elem) => ZIO.attempt(state + elem))
