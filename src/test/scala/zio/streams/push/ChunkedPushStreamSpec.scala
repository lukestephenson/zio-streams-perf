  ...
  override def spec = suite("ChunkedPushStreamSpec")(
    suite("range")(
      ...
      test("two large ranges can be concatenated") {
        assertZIO(
          (ChunkedPushStream.range(1, 1000, 10) ++ ChunkedPushStream.range(1000, 2000, 10)).runCollect
        )(equalTo(Chunk.fromIterable(Range(1, 2000))))
      },
      test("two small ranges can be concatenated") {
        assertZIO(
          (ChunkedPushStream.range(1, 10, 10) ++ ChunkedPushStream.range(10, 20, 10)).runCollect
        )(equalTo(Chunk.fromIterable(Range(1, 20))))
      },
      test("range emits no values when start >= end") {
        assertZIO(
          (ChunkedPushStream.range(1, 1, 10) ++ ChunkedPushStream.range(2, 1, 10)).runCollect
        )(equalTo(Chunk.empty))
      },
